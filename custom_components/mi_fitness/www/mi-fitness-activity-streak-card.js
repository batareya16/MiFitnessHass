class MiFitnessActivityStreakCard extends HTMLElement {
  constructor() {
    super();
  ...
  setConfig(config) {
    ...
    this._goal = parseInt(config.step_goal ?? 10000);
    this._name = config.name ?? "Step Goal";
  }

  _detectUid(hass) {
    // Find the account id from any mi_fitness entity so users don't need to
    // look up their Xiaomi user_id. Matches sensor.mi_fitness_<uid>_steps_today.
    const re = /^sensor\.mi_fitness_(.+)_steps_today$/;
  ...
  _fmtGoal() {
    return this._goal >= 1000
      ? `${(this._goal / 1000).toFixed(0)}k steps`
      : `${this._goal} steps`;
  }
  ...
  _byDate(history) {
    const m = {};
    for (const r of (history || [])) if (r.date) m[r.date] = r.steps || 0;
    return m;
  }
  ...
  _calcStreak(byDate) {
    ...
    for (let i = 0; i < 14; i++) {
      ...
      if (byDate[key] >= this._goal) { streak++; d.setDate(d.getDate() - 1); }
      else break;
    }
    return streak;
  }
  ...
  _calcBest(byDate) {
    ...
    for (const key of dates) {
      if (byDate[key] >= this._goal) { cur++; best = Math.max(best, cur); }
      else cur = 0;
    }
    return best;
  }
  ...
  _thisWeek(byDate) {
    ...
    for (let i = 0; i <= dow; i++) {
      ...
      const val = byDate[this._dateStr(d)];
      if (val !== undefined && val >= this._goal) count++;
    }
...
<ha-card>
  <div class="header">
    <div class="title">
      <ha-icon icon="mdi:walk" style="--mdc-icon-size:18px;color:var(--primary-text-color)"></ha-icon>
  ...
  <div class="chart-wrap">
    ...
    <svg class="chart-svg" width="100%" preserveAspectRatio="none" style="display:block;overflow:visible">
      <line class="goal-line" stroke="#ff9f43" stroke-width="0.6" stroke-dasharray="2,2" opacity="0.7" vector-effect="non-scaling-stroke"/>
    </svg>
  </div>
  ...
  _update() {
    const sensor  = this._hass.states[`sensor.mi_fitness_${this._uid}_steps_today`];
    const history = sensor?.attributes?.history_14d ?? [];
    const byDate  = this._byDate(history);

    const todaySteps = parseInt(sensor?.state) || 0;
    if (todaySteps > 0) {
      const todayKey = this._dateStr(new Date());
      byDate[todayKey] = Math.max(byDate[todayKey] ?? 0, todaySteps);
    }
    ...
    if (streak > 0) {
      ...
      badge.style.cssText = "font-size:12px;font-weight:500;color:#e08800";
    } else {
    ...
    this._dotEls.forEach(({ circle, svg, isToday }, i) => {
      ...
      const steps = byDate[key];
      const done  = steps !== undefined && steps >= this._goal;
      const hasData = steps !== undefined;

      circle.style.background = done ? "#ff9f43" : "var(--primary-background-color)";
      circle.style.border = isToday
        ? (done ? "2px solid #e08800" : "2px solid var(--secondary-text-color)")
      ...
      if (done) {
        ...
        svg.setAttribute("height", "12");
        svg.setAttribute("viewBox", "0 0 12 12");
        svg.innerHTML = `<polyline points="2,6 5,9 10,3" fill="none" stroke="#7a4000" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>`;
      } else if (hasData) {
        svg.style.display = "";
    ...
    // Chart
    const values  = days.map(d => byDate[this._dateStr(d)] ?? 0);
    const maxVal  = Math.max(this._goal * 1.4, ...values, 1000);
    const chartH  = 50;
    const goalPct = (this._goal / maxVal * chartH).toFixed(1);
    ...
    this._barRects.forEach(({ rect, lbl }, i) => {
      const val     = values[i];
      const met     = val >= this._goal;
      const h       = val > 0 ? Math.max(3, val / maxVal * chartH) : 0;
      const y       = chartH - h;
      const fill    = val === 0 ? "var(--divider-color)" : met ? "#ff9f43" : "var(--secondary-text-color)";
      const opacity = val === 0 ? "0.4" : "0.85";
      ...
      const text = this._fmtSteps(val);
      lbl.textContent = text;
      lbl.style.color = val === 0 ? "transparent" : met ? "#e08800" : "var(--secondary-text-color)";
...
if (!customElements.get("mi-fitness-activity-streak-card"))
  customElements.define("mi-fitness-activity-streak-card", MiFitnessActivityStreakCard);
window.customCards = window.customCards || [];
window.customCards.push({
  type:        "mi-fitness-activity-streak-card",
  name:        "Mi Fitness Activity Streak",
  description: "Daily step goal streak tracker with 7-day history chart",
});